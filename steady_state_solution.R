...
ik <- (1i*c(0:(length(g[-1])/2-1),0,(-length(g[-1])/2+1):-1))
...
birthPart <- function(psis){
    rev(2*L/(length(psis))*Re(
        fft(FqR*(fft(rev(k[-1]*psis))), inverse = TRUE)/(length(xvals[-1]))))
}

#Compute growth part via spectral differentiation

growthPart <- function(psis){
    (2*pi/L)*rev(Re(fft(fft(
        rev(g[-1]*psis))*ik,
