...
\newcommand{\x}{\tilde{x}}
\newcommand{\y}{\tilde{y}}
\newcommand{\wwmin}{\w_{\rm{min}}}
\newcommand{\xxmin}{\x_{\rm{min}}}
...
\cite{cuesta_sheldon_2016}. The model differs from other models in that it
...
effect of "limited similarity" which prevents similar species to coexist.
Even when such a model has a coexistence steady state, this steady state
...
In our model we describe the abundance density $\phy(w,\ws)$ of cells
of size $w$ and species $w*$, where a species is specified by the
maximum size $w*$ for cells of that species.
...
\label{eq:MFdiv}
\end{equation}
The first term on the right-hand side describes the dynamics of a growing organism as an extension of the McKendrick--von Foerster equation \cite{silvert:1978,silvert:1980} when $\gp(w,\ws)$ is the growth rate of a cell of size $\w$ and species $\ws$.
The second term is the rate at which cells of size $w$ are produced from the
division of cells of size $0<w'<\ws$.
It is composed from the rate $\dup(w',\ws)$ at which a cell of size $w'$ and species $\ws$ divides and the probability density $\splt(w|w')$ that the daughter cell has size $w$ given that its parent had size $w'$ when it divided.
The factor $2$ taking care of the fact that each parent cell yields two daughter cells.
...
The same equation describes this process for any species, hence $\ws$ enters as a parameter in every rate function involved.
...
weight. This has the advantage that for
all species this abundance has support in the same interval
$w/w_i\in[\wmin,1]$, were $\wmin$ is the smallest relative size possible
for any cell.

The reason for taking out the factor $w_i^{-\gamma}$ in \eqref{dis} is that it turns out that in the steady state this has the effect that all species are
...
The same scale-invariance property also implies that
\begin{align*}
...
&Q(w,w')&=q(w/w')/w'.
\end{align*}
If we substitute these expressions into the population balance equation \eqref{eq:MFdiv} and performing a change of variables to $\w =w/w_i$,
...
\end{split}\]
for all $i=1,\dots M$ and $\w\in[\wmin,1]$.

To numerically calculate the integral in \eqref{pbe} we make use of the fact that it is a convolution integral.
To see this we write $\w=e^\x$ and $\w'=e^\y$ and
...
$\xmin=\log\wmin$ is the smallest possible log cell size.
...
taking place, is that the factor $g_1(\y)$ is zero outside that interval anyway.
With $\y$ ranging only over the interval $[\xt,0]$, the largest region in
which the function $f_1$ gets evaluated is the interval $[\xmin, -\xt]$.
Thus we can replace $f_1$ by a periodic function $\bar{f_1}$ with period $L=-\xmin$, without changing the value
...
\end{equation}
where the positive first term comes form nutrient intake and the negative second term is the metabolic loss rate.
...
\end{equation}
where $\sigma(N,p)$ is the rate at which nutrients are consumed by the cells and $\rho(N)$ is the rate at which the resource is replenished.
...
eats another givem cell of size $w'$ is given by $S(w,w') = w^\nu s(w/w')$.
...
p_c(w)=\int p(w,\ws)d\ws=\sum_{i=1}^M w_i^{-\gamma}p_i(w/w_i)
=w^{-\gamma}\sum_{i=1}^M (w/w_i)^\gamma p_i(w/w_i)
=w^{-\gamma}\tilde{p}_c(w).
\end{equation}
...
\begin{equation}\label{Mp}
M_P(w,\ws)=w^{-\xi}\int s(w'/w)(w'/w)^{-\xi} \tilde{p}_c(w')w'^{-1}dw'
=w^{-\xi}C_2(\log w).
\end{equation}
Here $C_2(x)$ is a convolution integral:
\begin{equation}
...
We assume that the predation kernel $s(x)$ has
support only in a finite interval $[-\beta_P-\Delta_P/2, -\beta_P+\Delta_P/2]$
with the size $\Delta_P$ of that interval and the
typical predator/prey mass ration $\beta_P$ so that
$[-\beta_P-\Delta_P/2, -\beta_P+\Delta_P/2]\subset[\xmin,\xmax]$.
...
this becomes
\begin{equation}\label{Mp}
G_P(w,\ws)=\epsilon w^{1-\xi}\int s(w/w')(w/w')^{\gamma-2} p_c(w')w'^{-1}dw'
=\epsilon w^{1-\xi}C_3(\log(w)).
\end{equation}
Here $C_3(x)$ is the convolution integral with
\begin{equation}
