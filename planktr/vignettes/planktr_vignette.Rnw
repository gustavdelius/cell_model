...
The model differs from other planktonmodels in that it
...
In our model we describe the abundance density $\phy(w,\ws)$ of plankton cells
of size $w$ and species $\ws$, where a species is specified by the
